...
import com.example.demo.utils.db.DataTableDb;
import com.example.demo.utils.db.RowDb;
...
public class Backup {
    ...
    public String execute() throws SQLException {
        ...
        DataTableDb dataTable = null;

        // get tables
        tables = new TableCollection();
        dataTable = DataTableDb.parse(metaData.getTables(null, null, null, null));
        for (RowDb row : dataTable) {
            tables.add(new Table(row.getString("TABLE_NAME")));
        }

        // get columns
        for (Table table : tables) {
            dataTable = DataTableDb.parse(metaData.getColumns(null, null,
                    table.getName(), null));
        ...
        for (int i = 0; i < tables.size(); i++) {
            for (int j = 0; j < tables.size(); j++) {
                if (i != j) {
                    dataTable = DataTableDb.parse(metaData.getCrossReference(
        ...
        for (Table table : tables) {
            dataTable = DataTableDb.execute(connection, "select * from " + quote + table.getName() + quote);
            if (dataTable.size() > 0) {
    ...
    public String getSQLValue(Table table, RowDb row, int index) {
